...
$linkid = db_connect();
	...
	$aColumns = array();
//	$jurysignums = array('EVYZACA');
	$juryflag = 1;

	if(array_search(strtoupper($_REQUEST['signum']),$jurysignums) !== FALSE){
		$juryflag = 0;	
	}

	if($juryflag == 1){
		$aColumns = array('t1.Booth_ID','t1.Zone','t1.Booth_Number','t1.Type','t1.Title','Rate');
	}else{
		$aColumns = array('t1.Booth_ID','t1.Zone','t1.Booth_Number','t1.Type','t1.Title','t1.SPOC','Rate');
	}
	/* DB table to use */
	$sTable = 'booth';
...
/*
	 ...
	 */
	if($juryflag == 1){
		$sQuery = "SELECT SQL_CALC_FOUND_ROWS t1.Booth_ID,t1.Zone,t1.Booth_Number,t1.Type,t1.Title,concat(t2.Informative,'-',t2.Innovativeness,'-',t2.Overall_Presentability) as Rate FROM $sTable t1 left join voters t2 on t1.Booth_id = t2.Booth_ID and t2.Signum_ID='".$_REQUEST['signum']."' $sWhere $sOrder $sLimit";
	}else{
		$sQuery = "SELECT SQL_CALC_FOUND_ROWS t1.Booth_ID,t1.Zone,t1.Booth_Number,t1.Type,t1.Title,t1.SPOC,concat(t2.Informative,'-',t2.Innovativeness,'-',t2.Overall_Presentability) as Rate FROM $sTable t1 left join voters t2 on t1.Booth_id = t2.Booth_ID and t2.Signum_ID='".$_REQUEST['signum']."' $sWhere $sOrder $sLimit";
	}
