...
$query =  "select v.Booth_ID, b.Booth_Number, Zone, Type, Title, ROUND(avg(Informative) ,1) as Avg_Informative, "
       ...
       .  "count(v.Booth_ID) as vote_count from voters v join booth b on "
       .  "(v.Booth_ID = b.Booth_ID) group by b.Booth_Id "
...
$jury_result_query = "select booth_id, jury_score from jury_score";
$result = mysql_query($jury_result_query);
$jury_scores = array();
while ($row = mysql_fetch_object($result)) {
   $jury_scores[$row->booth_id] = $row->jury_score ; 
}
